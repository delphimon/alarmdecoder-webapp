...
decodersocket = Blueprint('sock', __name__)
...
class Decoder(object):
    ...
    def broadcast(self, channel, data={}):
        ...
        with self.app.app_context():
            self.websocket.emit(channel, obj, namespace='/alarmdecoder')
    ...
    def _test_open(self):
        ...
        try:
            ...
            self.broadcast('test', {'test': 'open', 'results': results, 'details': details})

    def _test_config(self):
        """Tests retrieving and saving the AlarmDecoder configuration."""
        def on_config_received(device):
            timer.cancel()
            self.broadcast('test', {'test': 'config', 'results': 'PASS', 'details': ''})
            if on_config_received in self.device.on_config_received:
                self.device.on_config_received.remove(on_config_received)

        def on_timeout():
            self.broadcast('test', {'test': 'config', 'results': 'TIMEOUT', 'details': 'Test timed out.'})
            if on_config_received in self.device.on_config_received:
                self.device.on_config_received.remove(on_config_received)
        ...
        try:
            with self.app.app_context():
                ...
                self.device.mode = panel_mode.value
                self.device.address = keypad_address.value
                self.device.address_mask = int(address_mask.value, 16)
                self.device.emulate_zone = zx
                self.device.emulate_relay = rx
                self.device.emulate_lrr = lrr_enabled.value
                self.device.deduplicate = deduplicate.value

                self.device.on_config_received += on_config_received
                self.device.save_config()
        except Exception as err:
            timer.cancel()
            if on_config_received in self.device.on_config_received:
                self.device.on_config_received.remove(on_config_received)
            self.broadcast('test', {'test': 'config', 'results': 'FAIL', 'details': 'There was an error sending the command to the device.'})
    ...
    def _test_send(self):
        ...
        def on_timeout():
            self.broadcast('test', {'test': 'send', 'results': 'TIMEOUT', 'details': 'Test timed out.'})
            if on_sending_received in self.device.on_sending_received:
                self.device.on_sending_received.remove(on_sending_received)
        ...
        timer.start()
        try:
            self.device.on_sending_received += on_sending_received
            self.device.send("*\r")
        except Exception as err:
            timer.cancel()
            if on_sending_received in self.device.on_sending_received:
                self.device.on_sending_received.remove(on_sending_received)
            self.broadcast('test', {'test': 'send', 'results': 'FAIL', 'details': 'There was an error sending the command to the device.'})
            self.app.logger.error('Error while testing keypad communication.', exc_info=True)

    def _test_receive(self):
        """Tests message received events."""
        def on_message(device, message):
            timer.cancel()
            if on_message in self.device.on_message:
                self.device.on_message.remove(on_message)
            self.broadcast('test', {'test': 'recv', 'results': 'PASS', 'details': ''})

        def on_timeout():
            self.broadcast('test', {'test': 'recv', 'results': 'TIMEOUT', 'details': 'Test timed out.'})
            if on_message in self.device.on_message:
                self.device.on_message.remove(on_message)
        ...
        timer.start()
        try:
            self.device.on_message += on_message
            self.device.send("*\r")
        except Exception as err:
            timer.cancel()
            if on_message in self.device.on_message:
                self.device.on_message.remove(on_message)
            self.broadcast('test', {'test': 'recv', 'results': 'FAIL', 'details': 'There was an error sending the command to the device.'})
            self.app.logger.error('Error while testing keypad communication.', exc_info=True)
...
@socketio_server.on('connect', namespace='/alarmdecoder')
def handle_connect():
    from flask import request as flask_request
    alarmdecoder = current_app.decoder
    try:
        session_interface = alarmdecoder.app.session_interface
        session = session_interface.open_session(alarmdecoder.app, flask_request)
        ...
        alarmdecoder.app.logger.error('Websocket connection failed: {0}'.format(err))

@socketio_server.on('keypress', namespace='/alarmdecoder')
...
@socketio_server.on('firmwareupload', namespace='/alarmdecoder')
def handle_firmwareupload(*args):
    alarmdecoder = current_app.decoder
...
@socketio_server.on('test', namespace='/alarmdecoder')
def handle_test(*args):
    alarmdecoder = current_app.decoder
    try:
        alarmdecoder._test_open()
        time.sleep(0.5)
        alarmdecoder._test_config()
        alarmdecoder._test_send()
        alarmdecoder._test_receive()
    except Exception:
